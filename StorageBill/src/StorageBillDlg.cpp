...
const wchar_t* g_arrHuoZhuName[] ={L"永创耀辉", L"弥雅食器", L"泰福商贸", L"颐麦科技", L"新马帮", L"七一酱园", L"永创昆仑山", L"凡将", L"韩太郎", L"玖王", L"至善电商", L"辣风芹", L"维敷康", L"硕果流香", L"静心阁", L"昆仑山水卡", L"阔伟电商", L"趣旅收纳"};
...
bool CStorageBillDlg::LoadXiaoShouChuKuDan(std::wstring wfileName, bool checkFail)
{
	...
	if(totalSheet)
	{
		...
		for(size_t r = 1; r < maxRows; ++r)
		{
			...
			if(dTemp > 0.001)
				dTemp += 0.05;
			if((_data.strWuLiuGongSi == L"顺丰热敏(线下)" || _data.strWuLiuGongSi == L"顺丰热敏(拼多多)"))
			{
...
bool CStorageBillDlg::_LoadYDKDData(const char *file)
{
	...
	if(ydSheet)
	{
		...
		if(!bFix)
		{
			...
			ydSheet->Cell(0, maxCols+1)->SetWString(L"对账备注1");
			ydSheet->Cell(0, maxCols+2)->SetWString(L"对账备注2");
		}
		else
		{
...
bool CStorageBillDlg::CheckYDKDMore()
{
	...
	while(itB1 != itE1)
	{
		...
		while(itB2 != itE2)
		{
			if(itB2->strWuLiuGongSi != L"韵达快递(拼多多)" && itB2->strWuLiuGongSi != L"韵达快递(菜鸟)")
			{
				++itB2;
				continue;
			}
...
bool CStorageBillDlg::CheckZTKYMore()
{
	...
	while(itB1 != itE1)
	{
		...
		while(itB2 != itE2)
		{
			if(itB2->strWuLiuGongSi != L"中通快运" && itB2->strWuLiuGongSi != L"中通快运(菜鸟)" && itB2->strWuLiuGongSi != L"中通快运(拼多多)")
			{
				++itB2;
				continue;
			}
...
bool CStorageBillDlg::CompareWithYDKDData(std::wstring strHuoZhu, std::list<sSalesInfo>& listInfo)
{
	if(!m_bSingleYDKD)
		return true;
	std::list<sSalesInfo>::iterator itYCBegin = listInfo.begin();
	...
	while(itYCBegin != itYCEnd)
	{
		if(itYCBegin->strWuLiuGongSi != L"韵达快递(菜鸟)" && itYCBegin->strWuLiuGongSi != L"韵达快递(拼多多)")
		{
			++itYCBegin;
			continue;
		}
		...
		if(itBS != m_mapYDKDAuthData.end())
		{
			...
			if(dYCWeight <= 0.3)
				...
				nYCWeight = (int)dYCWeight+1;
			int nBSWeight = itBS->second.finalWeight;
			if(nBSWeight > nYCWeight)
			{
				ydSheet->Cell(itBS->second.row, m_mapYDKDHandleCol[itBS->second.ydkdExcel].bzCol1)->SetDouble(nYCWeight);
				ydSheet->Cell(itBS->second.row, m_mapYDKDHandleCol[itBS->second.ydkdExcel].bzCol2)->SetWString(itYCBegin->strZhongLiang.c_str());
			}
		}
		++itYCBegin;
	}
...
bool CStorageBillDlg::CompareWithZTKYData(std::wstring strHuoZhu, std::list<sSalesInfo>& listInfo)
{
	...
	while(itYCBegin != itYCEnd)
	{
		if(itYCBegin->strWuLiuGongSi != L"中通快运" && itYCBegin->strWuLiuGongSi != L"中通快运(菜鸟)" && itYCBegin->strWuLiuGongSi != L"中通快运(拼多多)")
		{
			++itYCBegin;
			continue;
		}
		...
		if(sheet)
		{
			...
			while(itB != itE)
			{
				...
				//计算物流费
				{
					...
					else if(itB->strWuLiuGongSi == L"百世快运" || itB->strWuLiuGongSi == L"中通快运" || itB->strWuLiuGongSi == L"中通快运(菜鸟)" || itB->strWuLiuGongSi == L"中通快运(拼多多)")
					{
...
bool CStorageBillDlg::Handle_KunLunShanShuiKa()
{
	...
	string _file = CFuncCommon::WString2String(fileName.c_str());
	CompareWithSFData(L"昆仑山水卡", m_mapAllSalesInfo[L"昆仑山水卡"]);
	CompareWithBSKDData(L"昆仑山水卡", m_mapAllSalesInfo[L"昆仑山水卡"]);
	CompareWithZTKYData(L"昆仑山水卡", m_mapAllSalesInfo[L"昆仑山水卡"]);
	//计算相关费用
	{
		BasicExcelWorksheet* sheet = excel.GetWorksheet(L"订单费用");
		if(sheet)
...
bool CStorageBillDlg::Handle_KuoWeiDianShang()
{
	...
	string _file = CFuncCommon::WString2String(fileName.c_str());
	CompareWithSFData(L"阔伟电商", m_mapAllSalesInfo[L"阔伟电商"]);
	CompareWithBSKDData(L"阔伟电商", m_mapAllSalesInfo[L"阔伟电商"]);
	CompareWithZTKYData(L"阔伟电商", m_mapAllSalesInfo[L"阔伟电商"]);
	//计算相关费用
	{
		BasicExcelWorksheet* sheet = excel.GetWorksheet(L"订单费用");
		if(sheet)
...
bool CStorageBillDlg::Handle_JingXinGe()
{
	...
	string _file = CFuncCommon::WString2String(fileName.c_str());
	CompareWithSFData(L"静心阁", m_mapAllSalesInfo[L"静心阁"]);
	CompareWithBSKDData(L"静心阁", m_mapAllSalesInfo[L"静心阁"]);
	CompareWithZTKYData(L"静心阁", m_mapAllSalesInfo[L"静心阁"]);
	//计算相关费用
	{
		BasicExcelWorksheet* sheet = excel.GetWorksheet(L"订单费用");
		if(sheet)
		{
			...
			while(itB != itE)
			{
				...
				//计算物流费
				{
					if(itB->strWuLiuGongSi == L"顺丰热敏(线下)" || itB->strWuLiuGongSi == L"顺丰热敏(拼多多)" || itB->strWuLiuGongSi == L"韵达快递(拼多多)" || itB->strWuLiuGongSi == L"韵达快递(菜鸟)")
					{
						double money = 0;
						if(nWeight > 0)
						{
...
bool CStorageBillDlg::Handle_QuLvShouNa()
{
	...
	string _file = CFuncCommon::WString2String(fileName.c_str());
	CompareWithSFData(L"趣旅收纳", m_mapAllSalesInfo[L"趣旅收纳"]);
	CompareWithBSKDData(L"趣旅收纳", m_mapAllSalesInfo[L"趣旅收纳"]);
	CompareWithZTKYData(L"趣旅收纳", m_mapAllSalesInfo[L"趣旅收纳"]);
	//计算相关费用
	{
		BasicExcelWorksheet* sheet = excel.GetWorksheet(L"订单费用");
		if(sheet)
		{
			...
			while(itB != itE)
			{
				...
				//计算物流费
				{
					...
					else if(itB->strWuLiuGongSi == L"韵达快递(拼多多)" || itB->strWuLiuGongSi == L"韵达快递(菜鸟)")
					{
						double money = 0;
						if(nWeight > 0)
						{
...
bool CStorageBillDlg::Handle_ShuoGuoLiuXiang()
{
	...
	string _file = CFuncCommon::WString2String(fileName.c_str());
	CompareWithSFData(L"硕果流香", m_mapAllSalesInfo[L"硕果流香"]);
	CompareWithBSKDData(L"硕果流香", m_mapAllSalesInfo[L"硕果流香"]);
	CompareWithZTKYData(L"硕果流香", m_mapAllSalesInfo[L"硕果流香"]);
	//计算相关费用
	{
		BasicExcelWorksheet* sheet = excel.GetWorksheet(L"订单费用");
		if(sheet)
	...
	//计算相关费用
	{
		...
		if(sheet)
		{
			...
			while(itB != itE)
			{
				...
				//计算物流费
				{
					if(itB->strWuLiuGongSi == L"顺丰热敏(线下)" || itB->strWuLiuGongSi == L"百世快递(菜鸟)" || itB->strWuLiuGongSi == L"百世快递(拼多多)" || itB->strWuLiuGongSi == L"百世快递(线下)" || itB->strWuLiuGongSi == L"韵达快递(菜鸟)" || itB->strWuLiuGongSi == L"韵达快递(拼多多)" || itB->strWuLiuGongSi == L"中通快运(菜鸟)" || itB->strWuLiuGongSi == L"中通快运" || itB->strWuLiuGongSi == L"中通快运(拼多多)")
					{
